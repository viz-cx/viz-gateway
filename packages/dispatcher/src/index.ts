...
async function tick(
  ...
  for (const rec of due) {
    ...
    // (immutable) gross via the SAME static policy the signer's validateFeeSweep uses, so the
    // coordinator has no discretion. Only `base` is swept; any activation surcharge stays on
    // the gateway as backing surplus (recon credits it via the pinned row fee).
